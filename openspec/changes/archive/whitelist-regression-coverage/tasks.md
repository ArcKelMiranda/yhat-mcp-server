...
- **Aceptación**:
  - [x] Importa `classifyQuery` desde `../src/validator.js` o equivalente relativo del repo
  - [x] Cada escenario del spec tiene su propio `test(...)` en este archivo
  - [x] Los asserts usan `node:assert/strict` y el estilo del repositorio
  - [x] No se modifica ningún archivo de `src/`
  - [x] `npm test` pasa con todo en verde
...
- **Aceptación**:
  - [x] Importa `authorizeQueryTables` desde `../src/whitelist.js` o equivalente relativo
  - [x] Cada escenario del spec tiene su propio `test(...)` en este archivo
  - [x] Los asserts usan `node:assert/strict` y el estilo del repositorio
  - [x] No se modifica ningún archivo de `src/`
  - [x] `npm test` pasa con todo en verde
...
- **Aceptación**:
  - [x] La descripción del PR deja claro que es un cambio solo de pruebas
  - [x] La descripción indica que el comportamiento actual se fija como regresión para futuros cambios de parser/validador
  - [x] El PR queda listo para revisión
